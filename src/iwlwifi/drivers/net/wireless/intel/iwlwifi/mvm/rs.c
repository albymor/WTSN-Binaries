				...
				int rate_type)
{
	...
	low = index;
	while ((low != IWL_RATE_INVALID) && (low < IWL_RATE_COUNT)) {
	...
	high = index;
	while ((high != IWL_RATE_INVALID) && (high < IWL_RATE_COUNT)) {
				      ...
				      struct iwl_scale_tbl_info *tbl)
{
	...
	const struct rs_tx_column *column;
	if (tbl->column < (sizeof(rs_tx_columns)/sizeof(rs_tx_columns[0]))) {
                column = &rs_tx_columns[tbl->column];
        } else {
                column = &rs_tx_columns[(ARRAY_SIZE(rs_tx_columns) - 1)];
        }
			       ...
			       enum rs_column col_id)
{
	...
	const struct rs_tx_column *column;
	const struct rs_tx_column *curr_column;
	unsigned long rate_mask = 0;
	u32 rate_idx = 0;
	
	if ((col_id < (sizeof(rs_tx_columns)/sizeof(rs_tx_columns[0]))) && (tbl->column < (sizeof(rs_tx_columns)/sizeof(rs_tx_columns[0])))) {	
		column = &rs_tx_columns[col_id];
		curr_column = &rs_tx_columns[tbl->column];
	} else {
		column = &rs_tx_columns[(ARRAY_SIZE(rs_tx_columns) - 1)];
		curr_column = &rs_tx_columns[(ARRAY_SIZE(rs_tx_columns) - 1)];
	} 
				  ...
				  int tid, bool ndp)
{
	...
	/* Collect measured throughputs for current and adjacent rates */
	current_tpt = window->average_tpt;
	if ((low != IWL_RATE_INVALID) && (low < IWL_RATE_COUNT))
		low_tpt = tbl->win[low].average_tpt;
	if ((high != IWL_RATE_INVALID) && (high < IWL_RATE_COUNT))
